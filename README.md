### 👨‍💻 About Me

I am a **Full-Stack Developer** specializing in React, TypeScript, and modern PHP. Developing **Hybrid-Headless E-commerce platforms**—replacing legacy PHP/jQuery monolithic structures with highly reactive, DOM-hydrated React applications. 

* 📫 **Reach me at:** [m.yasir.inan@gmail.com](mailto:m.yasir.inan@gmail.com) | [LinkedIn](https://https://www.linkedin.com/in/myinan/)
* 📄 **Resume:** [View my CV here](#)

---

### 🛠️ Tech Stack & Tools

**Frontend Engineering**
<p align="left">
  <img alt="TypeScript" src="https://img.shields.io/badge/TypeScript-007ACC.svg?logo=typescript&logoColor=white">
  <img alt="React" src="https://img.shields.io/badge/React-20232a.svg?logo=react&logoColor=%2361DAFB">
  <img alt="Redux Toolkit" src="https://img.shields.io/badge/Redux_Toolkit-593D88?logo=redux&logoColor=white">
  <img alt="SASS/SCSS" src="https://img.shields.io/badge/Sass-CC6699?logo=sass&logoColor=white">
  <img alt="Zod" src="https://img.shields.io/badge/Zod-3E67B1?logo=zod&logoColor=white">
</p>

**Backend & Architecture**
<p align="left">
  <img alt="PHP" src="https://img.shields.io/badge/PHP_8+-777BB4.svg?logo=php&logoColor=white">
  <img alt="MySQL" src="https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white">
</p>

**DevOps & Tooling**
<p align="left">
  <img alt="Docker" src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white">
  <img alt="Vite" src="https://img.shields.io/badge/Vite-646CFF.svg?logo=vite&logoColor=white">
  <img alt="Git" src="https://img.shields.io/badge/Git-F05033.svg?logo=git&logoColor=white">
</p>

---

### Featured

#### 1. React-WordPress Hybrid E-Commerce Framework
This open-sourced boilerplate demonstrates how to build a hybrid-headless store. 
* **Key Features:** Custom PHP 8 Dependency Injection container, RTK Query integration with WooCommerce Store API, Zod form validation for local payment gateways (Iyzico, PayTR), and Vite HMR routing through a Dockerized PHP backend.
